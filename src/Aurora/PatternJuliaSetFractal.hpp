...
#include "EffectsLayer.hpp" // Ensure EffectsLayer is included
...
inline int32_t intfloat(float n){ return *(int32_t *)&n; }
// cast int32_t as float
inline float floatint(int32_t n){ return *(float *)&n; }
// fast approx sqrt(x)
inline float floatsqrt(float n){ return floatint(0x1fbb4000+(intfloat(n)>>1)); }
// fast approx 1/x
inline float floatinv(float n){ return floatint(0x7f000000-intfloat(n)); }
// fast approx log2(x)
inline float floatlog2(float n){ return (float)((intfloat(n)<<1)-0x7f000000)*5.9604645e-08f; }
#else
inline float floatinv(float n){ return 1.f/n;}
inline float floatsqrt(float n){ return std::sqrt(n); }
inline float floatlog2(float n){ return std::log2f(n); }
#endif
...
inline int32_t iteratefloat(float ax, float ay, float zx, float zy, uint16_t mxIT) {
  float zzl = 0;
...
class PatternJuliaSet : public Drawable {
  ...
  public:
    ...
    void drawPixelPalette(int x, int y, uint32_t m){
      ...
      } // Use effects object for drawing
      effects.leds[effects.XY16(x,y)] = CRGB(r,g,b);
